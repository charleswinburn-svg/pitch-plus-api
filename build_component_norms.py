...
Updated for split fastball/offspeed Stuff model. Computes PITCH-LEVEL mean
and std per pitch type (and globally) via streaming sums. Each component
(Stuff/Loc/Tun/Pitch+) can independently use one of three scoping modes:
  ...
  hybrid      global mean (cross-type comparable) + per-type std (within-type
              spread preserved). Recommended for Stuff+/Tun+/Pitch+.
...
class StreamingStats:
    def __init__(self):
        ...
        self.sum    = defaultdict(lambda: defaultdict(float))
        self.sumsq  = defaultdict(lambda: defaultdict(float))
